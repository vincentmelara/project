...
#
# Usage: Execute this script to generate the 3D t-SNE plot. Ensure the environment
...
if cbu_census_data.empty:
    ...
    # Prepare the 3D plot with CBU zip codes in light blue and top 50 non-CBU zip codes in yellow
    trace_cbu = go.Scatter3d(
        ...
        name='CBU'  # Add legend label for CBU trace
    )
    ...
    trace_non_cbu = go.Scatter3d(
        ...
        name='non-CBU'  # Add legend label for CBU trace
    )

    layout = go.Layout(
        title='3D t-SNE Map of ZIP Codes Based on Demographics',
        scene=dict(
