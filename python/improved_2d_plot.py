...
# Load the zip code similarities from the JSON file and limit to top 10 entries
with open('zip_code_similarities.json', 'r') as f:
    zip_data = json.load(f)[:10]  # Only take the top 10 objects
...
# Jitter coordinates to prevent exact overlaps
def jitter_coords(x, y, jitter_strength=0.01):
    return x + random.uniform(-jitter_strength, jitter_strength), y + random.uniform(-jitter_strength, jitter_strength)

coords_jittered = np.array([jitter_coords(x, y) for x, y in coords])

# Log that the plot is being built
logger.info("Building the 2D plot...")

# Plot 2D scatter plot
x, y = coords_jittered[:, 0], coords_jittered[:, 1]

# Draw the ZIP codes as nodes
...
# Draw lines (edges) between similar ZIP codes
edge_x = []
edge_y = []
...
for (zip1, zip2), sim in zip(zip_codes, similarities):
    ...
    edge_x.append(x[i])
    edge_x.append(x[j])
    edge_x.append(None)  # Separator for plotly line breaks
    edge_y.append(y[i])
    edge_y.append(y[j])
    edge_y.append(None)  # Separator for plotly line breaks

edges_trace = go.Scatter(
    x=edge_x,
    y=edge_y,
    mode='lines',
    line=dict(width=1, color='lightgreen'),  # Updated to light green
    hoverinfo='none'
)

layout = go.Layout(
    title='2D Similarity of ZIP Codes with t-SNE (Top 10)',
...
# Show the 2D plot
logger.info("Displaying the 2D plot with connections (Top 10)...")
fig.show()
